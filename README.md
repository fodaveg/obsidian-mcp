...
| `OBSIDIAN_MCP_ENABLE_EXEC` | If `1`, registers the `obsidian_exec` tool (see security below) | (empty) |
| `OBSIDIAN_MCP_DISABLE_EXEC` | If `1`, removes the `obsidian_exec` tool even when the one above is set | (empty) |
